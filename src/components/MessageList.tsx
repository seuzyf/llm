                              ...
                              <div className="flex flex-wrap gap-2">
                                {message.citations.map((cit, idx) => (
                                  <button
                                    ...
                                    className="bg-[#2a2b3d] hover:bg-[#313244] shrink-0 inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-600 text-sm shadow-sm transition-all"
                                  >
                                    {cit.type === 'file' ? <FileText size={14} className="text-cyan-400" /> : <History size={14} className="text-blue-400" />}
                                    <span className="truncate max-w-[150px] text-gray-300" title={cit.name}>{cit.name}</span>
                                  </button>
                                ))}
                              </div>
                            </div>
      ...
      {activeCitation && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#181825] border border-gray-700 rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            <div className="flex justify-between items-center p-4 border-b border-gray-800 bg-[#1e1e2e]">
              <h3 className="font-medium text-gray-200 flex items-center gap-2">
                {activeCitation.type === 'file' ? <FileText size={18} className="text-cyan-400" /> : <History size={18} className="text-blue-400" />}
                <span className="truncate max-w-[400px]" title={activeCitation.name}>
                  {activeCitation.type === 'file' ? '文件片段：' : '对话片段：'}{activeCitation.name}
                </span>
              </h3>
              <button onClick={() => setActiveCitation(null)} className="text-gray-400 hover:text-red-400 transition-colors"><X size={20} /></button>
            </div>
            
            <div className="p-5 overflow-y-auto whitespace-pre-wrap text-sm text-gray-300 leading-relaxed font-mono bg-[#1e1e2e]/30">
            ...
            {activeCitation.type === 'file' && activeCitation.url && (
              <div className="p-4 border-t border-gray-800 bg-[#1e1e2e] flex justify-end">
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer" 
                  className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors shadow-lg"
                >
